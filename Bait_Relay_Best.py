...
STD_START = 0.3
STD_END = 0.05
STD_DECAY_EPISODES = 1000  # how many episodes to fully decay over


def get_exploration_std(episode):
    fraction = min(episode / STD_DECAY_EPISODES, 1.0)
    return STD_START - (STD_START - STD_END) * fraction
...
class Fish:
    ...
    def move(self, brain, shark_position, border, fishes, exploration_std):
        ...
        nearest_shark_edge = min(shark_left, shark_right, shark_top, shark_bottom)
        ...
        fish_to_shark_distance = np.hypot(fish_to_shark_dx, fish_to_shark_dy)
        relay_distance = fish_to_shark_distance / np.hypot(WIDTH, HEIGHT)
        relay_alignment = 0
        if fish_to_shark_distance > 1e-3:
            relay_alignment = np.dot(
                np.array([fish_to_shark_dx / fish_to_shark_distance,
        ...
        if closest_teammate_distance == np.inf:
        ...
        angle_distribution = torch.distributions.Normal(
            loc=angle_mean,
            scale=torch.tensor(exploration_std)
        )
    ...
    def learn(self, hunted, won, shark_edge_change, shark_following, shark_position,
             in_danger_zone, clean_handoff):
        self.reward = 0
        if self.alive:
            self.reward += 0.05

        # Danger zone penalty for not bait
        if self.alive and not shark_following and in_danger_zone:
            self.reward -= 0.5
        ...
        if not shark_following and self.alive and self.relay_alignment > 0.5:
            ...
            if shark_edge_change > 0:
                self.reward += 1.0 * shark_edge_change
        if not shark_following and self.alive and self.relay_alignment <= 0.5:
            self.reward += 0.05 * (shark_to_fish_distance / np.hypot(WIDTH, HEIGHT))

        # Death Penalties (rebalanced -- this is the change that got you to 80%)
        if hunted:
            self.reward -= 20
...
class Border:
    def __init__(self):
        self.thickness = 20
        self.top = pygame.Rect(0, 0, WIDTH, self.thickness)
        self.bottom = pygame.Rect(0, HEIGHT - self.thickness, WIDTH, self.thickness)
        self.left = pygame.Rect(0, 0, self.thickness, HEIGHT)
        self.right = pygame.Rect(WIDTH - self.thickness, 0, self.thickness, HEIGHT)
...
def run_instance(fish_brain, exploration_std):
    fishes = []
    for i in range(45):
    ...
    previous_bait_failed = False
    in_danger_zone_last_frame = {}
    ...
    while sim:
        ...
        for fish in fishes:
            if fish.alive and shark.alive:
                fish.move(fish_brain, shark.position, collision, fishes, exploration_std)
                fish.draw()
        ...
        for fish in fishes:
            ...
            if fish.took_action:
                ...
                shark_following = (closest is fish)
                dist_to_shark = np.hypot(fish.position[0] - shark.position[0],
                                         fish.position[1] - shark.position[1])
                in_danger_zone = dist_to_shark < DANGER_ZONE_RADIUS

                clean_handoff = (
                    shark_following
                    and previous_bait_failed
                    and fish is not previous_closest
                    and not in_danger_zone_last_frame.get(id(fish), False)
                )
                ...
                if fish is closest:
                    in_danger_zone_last_frame[id(fish)] = in_danger_zone

        previous_bait_failed = (previous_closest is not None and not previous_closest.alive and shark.alive)
...
ax2.set_title(f"Shark Death Rate (Rolling {ROLLING_WINDOW}-Episode Window)")
ax2.set_ylim(0, 100)
...
loss_line, = ax4.plot([], [], color='orange', linewidth=0.7)
ax4.set_xlabel("Episode Number")
ax4.set_ylabel("Loss")
ax4.set_title("Loss over Episode Number")
...
for i in range(5000):
    exploration_std = .2
    pygame.display.set_caption("Mr Fish Simulation " + str(i))
    fishes, quit_requested, frame_count, shark_died, survival_percentage = run_instance(fish_brain, exploration_std)
    loss, avg_reward = train_instance(fishes, optimizer)
    ...
    loss_plotting.append(loss)

    recent_flags = shark_died_flags[-ROLLING_WINDOW:]
    shark_death_rate = (sum(recent_flags) / len(recent_flags)) * 100
    ...
    reward_line.set_data(episode_numbers, avg_rewards)
    ax3.relim()
    ax3.autoscale_view()

    loss_line.set_data(episode_numbers, loss_plotting)
    ax4.relim()
    ax4.autoscale_view()
